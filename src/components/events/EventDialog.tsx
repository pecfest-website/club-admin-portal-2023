...
const EventDialog = ({ onClose, open, user_token }: any) => {
    ...
    const [eventVenue, setEventVenue] = useState<any>();
    const [minTeamSize, setMinTeamSize] = useState<any>();
    const [maxTeamSize, setMaxTeamSize] = useState<any>();
    const [rulesLink, setRulesLink] = useState<any>();
    ...
    const [pocNumber, setPocNumber] = useState<any>();
    ...
    const [eventCreationStatus, setEventCreationStatus] = useState<any>();
    const [teamSizeError, setTeamSizeError] = useState(false);
    const [isLoading, setIsLoading] = useState(false);

    const handleEventChange = (e: any, type: any) => {
        if ("$d" in e) {
            if (type == 0) {
                if (e["$d"] > eventEnd) {
                    setDateError(true);
            ...
            switch (target_name) {
                ...
                case "maxTeamSize":
                    if (
                        minTeamSize &&
                        Number(minTeamSize) > Number(target_value)
                    ) {
                        setTeamSizeError(true);
                    } else {
                        setTeamSizeError(false);
    ...
    const handleEventSubmit = async (e) => {
        ...
        if (!dateError && !teamSizeError && eventPoster) {
            ...
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_API}club/`,
                {
                    method: `POST`,
                    headers: {
                        ...
                        {eventType == `TEAM` && teamSizeError && (
                            <Grid item>
                                <Alert severity="error">
                                    Min Team Size should be less than Max Team
                                    Size. 😐
                                </Alert>
                            </Grid>
                        )}
                        ...
                        <Grid item xs={12} sm={12}>
                            <Button fullWidth variant="contained" type="submit">
                                {!isLoading ? (
                                    ...
                                    <RestartAlt className={styles.loader} />
                                )}
                            </Button>
                        </Grid>
                    </Grid>
